...
describe('ArbitratorManifest', () => {
  ...
  it('only allows subject to set parties', async () => {
    ...
    await expectRevert(
      ...
      'ArbitratorManifest: not subject'
    )
  ...
  it('can override parties', async () => {
    const disputeId = safeBN(0)
    const receipt = await this.disputable.setPartiesOf(disputeId, user2, user1, { from: attacker1 })
    expectEvent.inTransaction(receipt.tx, this.manifest, 'PartiesSet', {
      disputeId,
      defendant: user2,
      challenger: user1
  ...
  it('allows setting enabled representative', async () => {
